...
def _remove_outliers(
    ...
    """
    z_scores = np.abs((X - np.mean(X, axis=axis)) / np.std(X, axis=axis))
    outliers = np.any(z_scores > max_z_score, axis=1 if axis == 0 else 0)
...
def _flatten_snapshots(X: np.ndarray) -> np.ndarray:
    """
    Flatten the snapshots in the data matrix into 2D arrays.
...
def _unflatten_snapshots(X: np.ndarray, shape_X: Tuple[int, int]) -> np.ndarray:
    ...
    - shape_X (Tuple[int, int]): Original dimensions of snapshots.
